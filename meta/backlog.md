...
- [ ] **R03 — `meta/compositions/` directory does not exist** (Owner: designer · Effort: S)
  - Source: pouk-ai-designer
  - Where: `meta/compositions/` — absent
  - Why: Pipeline §3 requires a composition recipe at `meta/compositions/pages/<route>.md` for every page. Currently empty for all four routes — blocks the Designer-stage `Approved` gate site-wide.

- [ ] **R04 — No Approved canonical composition for /** (Owner: designer · Effort: M)
  - Source: pouk-ai-designer
  - Where: `meta/compositions/pages/home.md` — would-be path; absent
  - Why: Homepage was built by the engineer without an upstream composition. Future Hero internal-spacing or token changes in `@poukai-inc/ui` will land on `/` with no review gate. Designer should author the recipe ratifying current DS-default rhythm + zero-JS posture.

- [ ] **R05 — No Approved canonical content draft for /** (Owner: content · Effort: M)
  - Source: pouk-ai-content (orchestrator-inline)
  - Where: `meta/content/drafts/pages/home.md` — absent
  - Why: Per pipeline §3 (`pouk-ai-content`), every page route requires an Approved draft before composition/engineering. Full copy audit can only be partial without one; future copy revisions have no canonical record to diverge from.
